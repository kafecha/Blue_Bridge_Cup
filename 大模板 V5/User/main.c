...
unsigned char Wave_Data;//超声波数据储存变量
...
void Read_Proc()
{
	unsigned char Filter_Dat;//滤波前数据接收变量
	switch(Proc_Flag)
	{
		case 4://超声波读取
			Filter_Dat = Ut_Wave_Data();//读取超声波数据
			Wave_Data = Median_Filter(Filter_Dat);//滑动平均数滤波处理
			Proc_Flag = 0;//清除标志位状态
		break;
	}
}
...
{  
	...
	if(Proc_Slow_Down % 9) Proc_Flag = 1;
